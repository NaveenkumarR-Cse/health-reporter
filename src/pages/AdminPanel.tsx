...
const AdminPanel = () => {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-heading font-bold">Manage <span className="gradient-text">Users</span></h1>
        <p className="text-muted-foreground mt-1">Add community users and manage system access</p>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-8">
        <div className="glass-card p-6">
          ...
          <p className="text-sm text-muted-foreground mb-4">Community users can login to view health data and receive alerts.</p>
          <form onSubmit={handleAddCommunity} className="space-y-3">
            <div><Label>Full Name</Label><Input placeholder="Enter name" className="mt-1" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></div>
            <div><Label>Email</Label><Input type="email" placeholder="user@example.com" className="mt-1" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required /></div>
            <div><Label>Mobile Number</Label><Input placeholder="+91 XXXXXXXXXX" className="mt-1" value={form.mobile} onChange={(e) => setForm({ ...form, mobile: e.target.value })} required /></div>
            <div><Label>Village</Label><Input placeholder="Village name" className="mt-1" value={form.village} onChange={(e) => setForm({ ...form, village: e.target.value })} required /></div>
            <div><Label>Password</Label><Input type="password" placeholder="Set password" className="mt-1" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required /></div>
            <Button type="submit" className="w-full gap-2"><UserPlus className="w-4 h-4" /> Add Community User</Button>
          </form>
        </div>
